    ...
    $(document).ready(function($) {
        ...
        function setupMapa(div){
          ...
          // $.post('http://ttaudit.com/getPointStoresForCompanyDepartament',{ company_id : <?php echo $_GET['company_id'] ?>,departament : "<?php echo $_GET['departament'] ?>" }, function(json){
           $.post('http://localhost/ttaudit.com/backend/ttaudit1/public/JsonPrueba',{ company_id : <?php echo $_GET['company_id'] ?>,departament : "<?php echo $_GET['departament'] ?>" }, function(json){
...
//                );
        ...
        function populateMap(data){
            ...
            function openInfoBox(marker, item) {
                ...
                var myOptions = {
                    content 			:createInfoboxHtml(item.cadenaRuc,item.tipo,item.codclient, item.fullname, item.departamento, item.address, item.referencia, item.district ,item.id)
                    ,alignBottom 			:true
            ...
            function createInfoboxHtml(cadenaRuc,tipo,codclient,fullname, departamento, address, referencia,district,id) {
