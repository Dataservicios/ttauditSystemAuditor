...
$poll_ids_limits = array(13,17,18,19);
...
/* Preguntas que tienen solo Opciones */
...
if (in_array($polls_id, $poll_ids_options_no_limit)) {
	// Create new PHPExcel object
    $objPHPExcel = new PHPExcel();
    $objWorksheet = $objPHPExcel->getActiveSheet();
	...
	if ($totEmp> 0) {
	    ...
	    while ($rowEmp = mysql_fetch_assoc($resEmp)) {
	        ...
	        $objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue('A'.$conatador, utf8_encode($rowEmp['store_name']) )
	            ->setCellValue('B'.$conatador, utf8_decode($rowEmp['type_options']) ) ;

	        if ($rowEmp['result'] == '0') {  	

				    $objPHPExcel->setActiveSheetIndex(0)
				        ->setCellValue('C4', 'OTRO');
	    ...
	    $query = "SELECT * FROM poll_options  where poll_id=". $polls_id ;
...
*/
if (in_array($polls_id, $poll_ids_limits)) {	
	// Create new PHPExcel object
    $objPHPExcel = new PHPExcel();
    $objWorksheet = $objPHPExcel->getActiveSheet();
	...
	if ($totEmp> 0) {
	    ...
	    while ($rowEmp = mysql_fetch_assoc($resEmp)) {
        	...
        	$limite = utf8_decode($rowEmp['limite']);

        	if ($polls_id == 13) {
        		# code...
        		$limite_desc = substr($limite, strpos($limite,"|") + 1, strlen($limite) - strpos($limite,"|") - 1);
        	}else{
        		$limite_desc = $limite;
        	}
	        ...
	        $objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue('A'.$conatador, utf8_encode($rowEmp['store_name']) )
	            ->setCellValue('B'.$conatador, utf8_encode($limite_desc)  ) ;
	     ...
	     $query = "SELECT * FROM poll_ranges  where poll_id=". $polls_id ;
        $resQuery = mysql_query($query, $conexion_db) or die(mysql_error());
        $totQuery = mysql_num_rows($resQuery);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('G3', 'RESUMEN')
                ->setCellValue('G4', 'TIPO')
                ->setCellValue('H4', 'Valores');


        while ($rowQuery = mysql_fetch_assoc($resQuery)) {
	        $contadorTipo ++ ;
	        $contadorFilas ++;
	        $objPHPExcel->getActiveSheet()->setCellValue('G'.$contadorTipo , utf8_encode($rowQuery['nombre']))
	            ->setCellValue('H'.$contadorTipo, '=COUNTIF(B5:B'.$conatador.',"'.utf8_encode($rowQuery['nombre']).'")');

	        $objPHPExcel->getActiveSheet()->getCell('I'.$contadorTipo)->getCalculatedValue();

	    }
			$dataSeriesLabels = array(
            new PHPExcel_Chart_DataSeriesValues('String', 'REPORTE1!$G$4', NULL, 1),	//	2010
        );
        //GENERANDO GRÁFICO
        $xAxisTickValues = array(
            new PHPExcel_Chart_DataSeriesValues('String', 'REPORTE1!$G$5:$G$'.$contadorTipo , NULL, $contadorFilas),	//	Q1 to Q4
        );

        $dataSeriesValues = array(
            new PHPExcel_Chart_DataSeriesValues('Number', 'REPORTE1!$H$5:$H$'.$contadorTipo, NULL, $contadorFilas),
        );
        //	Build the dataseries
        $series = new PHPExcel_Chart_DataSeries(
            PHPExcel_Chart_DataSeries::TYPE_BARCHART,		// plotType
...
//            // DEFINE COLOR DE TEXTO
            ...
            $objPHPExcel->getActiveSheet()->getStyle('G4:H5')->getFill()->getStartColor()->setARGB('2080D0');

//
//            //APLICANDO BORDE A LAS CELDAS
            $objPHPExcel->getActiveSheet()->getStyle('G4:H'. $contadorTipo + 1)->applyFromArray($styleThinBlackBorderOutline);
//
//
